...
class _DeleteDependentPageState extends State<DeleteDependentPage> {
  ...
  Future<void> _deleteDependent() async {
    final result = await _applicantsController.deleteDependent(
      widget.dependentId,
      widget.applicantId,
    );
      ...
      result.fold(
        (success) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('${_dependent?.name} excluído com sucesso!'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarCustom(
        ...
        path: RoutePaths.ptd.dependentId(
          widget.applicantId,
          widget.dependentId,
        ),
      ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Tem certeza que deseja excluir o dependente ${_dependent?.name}?',
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'CPF: ${_dependent?.cpf ?? 'N/A'}',
                          style: const TextStyle(fontSize: 14),
                        ),
                        const SizedBox(height: 8),
                        Text(
                          'Email: ${_dependent?.email ?? 'N/A'}',
                          style: const TextStyle(fontSize: 14),
                        ),
                        const SizedBox(height: 8),
                        Text(
                          'Responsável: ${_applicantName ?? 'N/A'}',
                          style: const TextStyle(fontSize: 14),
                        ),
                      ],
                    ),
