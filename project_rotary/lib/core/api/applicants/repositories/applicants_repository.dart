...
class ApplicantsRepository {
  ...
  AsyncResult<List<Applicant>> getBeneficiaryApplicants() async {
    final filters = ApplicantFilters(isBeneficiary: true);
    return getApplicants(filters: filters);
  }

  /// Busca applicants não beneficiários
  AsyncResult<List<Applicant>> getNonBeneficiaryApplicants() async {
    final filters = ApplicantFilters(isBeneficiary: false);
    return getApplicants(filters: filters);
  }

  /// Busca dependents de um applicant específico
  AsyncResult<List<Dependent>> getDependentsByApplicant(
    String applicantId,
  ) async {
    final filters = DependentFilters(applicantId: applicantId);
    return getDependents(filters: filters);
  }

  /// Busca applicants por nome (busca parcial)
  AsyncResult<List<Applicant>> searchApplicantsByName(String name) async {
    final filters = ApplicantFilters(name: name);
    return getApplicants(filters: filters);
  }

  /// Busca applicants por CPF
  AsyncResult<List<Applicant>> searchApplicantsByCpf(String cpf) async {
    final filters = ApplicantFilters(cpf: cpf);
    return getApplicants(filters: filters);
  }

  /// Busca applicants por email
  AsyncResult<List<Applicant>> searchApplicantsByEmail(String email) async {
  ...
  /// Busca dependents por nome (busca parcial)
  AsyncResult<List<Dependent>> searchDependentsByName(String name) async {
    final filters = DependentFilters(name: name);
    return getDependents(filters: filters);
  }

  /// Busca dependents criados em um período
